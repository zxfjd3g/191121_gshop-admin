        ...
        <el-table border :data="attrs">
          ...
          <el-table-column label="属性值列表">
            <template slot-scope="{row, $index}">
              <el-tag style="margin: 2px" type="info" v-for="value in row.attrValueList" 
  ...
  methods: {
    ...
    */
    toEdit (value, index) {
      ...
      if (value.hasOwnProperty('edit')) { // hasOwnProperty判断对象自身上是否有某个属性
        value.edit = true
      } else {
        // 如果没有, 必须通过$set()来添加一个新属性
        this.$set(value, 'edit', true)   // $set / $nextTick()
      }
    ...
    */
    showUpdate (attr) {  // attr.attrValueList
      // 保存要修改的属性对象
